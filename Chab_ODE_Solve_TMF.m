function [state_var]=Chab_ODE_Solve_TMF(x,ideal_load_cycle_data,Rel_Tol)
...
for jj=1:1:size(ideal_load_cycle_data,1)
    ...
    for ii=2:1:size(ideal_load_cycle_data_temp,1)
        ...
        y0=[(state_var_1(ii-1,1)/100),(state_var_1(ii-1,2)/100),state_var_1(ii-1,4),state_var_1(ii-1,5),state_var_1(ii-1,6),(state_var_1(ii-1,7)/100),(state_var_1(ii-1,8)/100),(state_var_1(ii-1,9)/100),state_var_1(ii-1,3)];
        temperature=ideal_load_cycle_data_temp(ii,3);
        temperature_rate=(ideal_load_cycle_data_temp(ii,3)-ideal_load_cycle_data_temp(ii-1,3))/((t_final-t0));
        [t,y]=ode45('Chab_ODEs_TMF',[t0,t_final],y0,options,x,strain_rate,temperature,temperature_rate); %Call ODE45 to solve Chaboche ODEs
        state_var_1(ii,1)=y(end,1)*100;%Plastic Strain
        state_var_1(ii,2)=y(end,2)*100;%Accumulated Plastic Strain
        state_var_1(ii,3)=y(end,9);%Drag Stress
