function [ideal_load_cycle_data_2]=ideal_load_cycle_gen_2(strain_rate)

sec_n=20;

turn_point_times=[0.5/strain_rate;
                  0.5/strain_rate;
                  0.5/strain_rate;
                  0.5/strain_rate];
...
turn_point_strain=[0.5;
 ...
 for ii=1:1:size(turn_point_times)
    if ii==1
        ...
        ideal_load_cycle_data_2(1:(sec_n+1),1)=transpose(linspace(t0,turn_point_times(ii,1),sec_n+1));
        ideal_load_cycle_data_2(1:(sec_n+1),2)=transpose(linspace(e0,turn_point_strain(ii,1),sec_n+1));
        clear t0 e0
    else
        if turn_point_strain(ii,1)==turn_point_strain(ii-1,1)
            ...
            ideal_load_cycle_data_2((((ii-1)*sec_n)+2):((ii*sec_n)+1),1)=t_temp(2:end);
            ideal_load_cycle_data_2((((ii-1)*sec_n)+2):((ii*sec_n)+1),2)=e_temp(2:end);
